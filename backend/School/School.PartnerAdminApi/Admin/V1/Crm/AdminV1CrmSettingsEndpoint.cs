...
public sealed class AdminV1CrmSettingsEndpoint : IEndpointMarker
{
        ...
        IPermissionService perms, CancellationToken ct)
    {
        ...
        var ok = await perms.HasAsync(httpContext.User, AdminPermissions.CrmSettings, ct);
    ...
    private static async Task<IResult> PipelineCreateAsync(
        ...
        UserManager<ApplicationUser> userManager, IPermissionService perms, CancellationToken ct)
    {
        if (await RequireAdministratorAsync(httpContext, userManager, perms, ct) is { } fail) return fail;
