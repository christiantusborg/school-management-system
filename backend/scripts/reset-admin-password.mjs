...
// Encrypt fields (matching C# FieldEncryption.Encrypt)
const encryptedOprfSeed      = encryptField(Buffer.from(oprfSeed))
const encryptedClientPubKey  = encryptField(Buffer.from(clientPublicKey))
...
if (existing) {
  db.prepare(`
    ...
    WHERE UserId = ?
  `).run(encryptedOprfSeed, encryptedClientPubKey, userId)
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, datetime('now'))
  `).run(newId, userId, encryptedOprfSeed, encryptedClientPubKey)
