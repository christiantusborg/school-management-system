          ...
          <div v-for="sec in prof.sections" :key="sec.id" class="ft-section">
            ...
            <template v-if="sec.kind === 'fields'">
              <div v-for="f in sec.fields" :key="f.id" class="ft-field">
                ...
                <div v-else-if="f.type === 'file' || f.type === 'files'" class="ft-files">
                  <div v-for="(fi, i) in cellFiles(cell(fieldsRow(sec), f.id))" :key="i" class="ft-file">
                ...
                <tbody>
                  <tr v-for="(row, ri) in gridRows(sec)" :key="ri">
                    <td v-for="f in sec.fields" :key="f.id">
                      ...
                      <div v-else-if="f.type === 'file' || f.type === 'files'" class="ft-files">
                        <div v-for="(fi, i) in cellFiles(cell(row, f.id))" :key="i" class="ft-file">
                          <span class="ft-file-ok">✓ {{ fi.name || 'file' }}</span>
...
async function downloadFilesItem(c, index, item) {
  ...
  try {
    ...
    const path = item?.legacy
      ? `/v1/admin/faculty-values/${c.valueId}/file`
      : `/v1/admin/faculty-values/${c.valueId}/file/${index}`
