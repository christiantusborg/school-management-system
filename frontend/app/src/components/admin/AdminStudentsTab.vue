...
async function openGradeReview(s, e) {
  ...
  try {
    ...
    gradeModal.value.subjects = (res.data.items ?? []).map(withRubricScores)
  } catch (err) {
...
async function confirmGradeSubmission() {
  ...
  try {
    await api.post(
      ...
      { items: scoredRows(m).map(rubricAwareEntry).filter(Boolean), projectTitle: m.projectTitle })
...
const EXPORT_FIELD_GROUPS = [
  ...
  { id: 'enrolmentDetail', label: 'Enrolment detail — switches to row-per-enrolment', fields: [
    ...
    { id: 'pathwayName',         label: 'Entry Requirement' },
    { id: 'pathwayMinYearsExp',  label: 'Entry Requirement min yrs exp' },
