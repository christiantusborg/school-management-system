<template>
  <div class="schools-root">
    <div class="crud-header">
      <div>
        <h2 class="crud-title">Schools</h2>
        <p class="crud-sub" v-if="!loading">{{ activeCount }} school{{ activeCount !== 1 ? 's' : '' }}</p>
      </div>
      <div class="head-actions">
        <label class="show-deleted"><input type="checkbox" v-model="showDeleted" @change="load" /> Show deleted</label>
        <button class="btn-primary" @click="openCreate">+ Add School</button>
      </div>
    </div>
    ...
    <div class="table-wrap">
      <table class="data-table">
        <thead>
          <tr><th>Name</th><th>Description</th><th>Programmes</th><th>Actions</th></tr>
        </thead>
        <tbody>
          <tr v-if="loading"><td colspan="4" class="empty-row">Loading…</td></tr>
          <tr v-else-if="items.length === 0"><td colspan="4" class="empty-row">No schools yet.</td></tr>
          <tr v-for="s in items" :key="s.schoolId" class="data-row" :class="{ 'row-deleted': s.deletedAt }">
            <td>{{ s.name }}<span v-if="s.deletedAt" class="del-badge">deleted</span></td>
            <td>{{ s.description || '—' }}</td>
            <td>{{ s.programmeCount }}</td>
            <td class="actions-cell">
              <template v-if="!s.deletedAt">
                <button class="btn-action" @click="openEdit(s)">Edit</button>
                <button class="btn-action btn-action-danger" @click="confirmDelete = s">Delete</button>
              </template>
              <button v-else class="btn-action" @click="restore(s)">Restore</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <template v-if="showForm">
      ...
      <div class="drawer">
        <div class="drawer-header">
          <h2>{{ editTarget ? 'Edit' : 'Add' }} School</h2>
          <button class="drawer-close" @click="showForm = false">✕</button>
        </div>
        <div class="drawer-form">
          <div class="field">
            <label>Name <span class="req">*</span></label>
            <input v-model="form.name" placeholder="e.g. International Business School of Scandinavia (IBSS)" />
          </div>
          <div class="field">
          ...
          <p v-if="editTarget" class="mail-hint">
            This school's outbound mail service is configured in System Config → Email
            (pick the school in the "Configure for" selector).
          </p>
...
const activeCount = computed(() => items.value.filter(s => !s.deletedAt).length)

async function load() {
  loading.value = true
  error.value = null
  try {
    const res = await api.get(`/v1/school/schools?includeDeleted=${showDeleted.value}`)
...
function openEdit(s) {
  editTarget.value = s
  form.name = s.name; form.description = s.description ?? ''
  formError.value = null
  showForm.value = true
}
...
async function save() {
  ...
  try {
    ...
    if (editTarget.value) await api.put(`/v1/school/schools/${editTarget.value.schoolId}`, body)
    else await api.post('/v1/school/schools', body)
...
async function doDelete(s) {
  ...
  error.value = null
  try {
    await api.delete(`/v1/school/schools/${s.schoolId}`)
    await load()
  } catch (e) {
...
async function restore(s) {
  error.value = null
  try {
    await api.post(`/v1/school/schools/${s.schoolId}/restore`)
    await load()
  } catch (e) {
    error.value = e.response?.data?.message ?? e.message ?? 'Restore failed'
  }
}
...
.req { color: #c0392b; }
.field input, .field textarea { padding: 0.58rem 0.75rem; border: 1.5px solid #ccc; border-radius: 7px; font-size: 0.9rem; font-family: inherit; outline: none; resize: vertical; }
.field input:focus, .field textarea:focus { border-color: #0055a5; }
.mail-hint { font-size: 0.78rem; color: #777; margin: 0; line-height: 1.45; }
