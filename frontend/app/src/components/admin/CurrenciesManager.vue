<template>
  <div class="schools-root">
    <div class="crud-header">
      <div>
        <h2 class="crud-title">Currencies</h2>
        <p class="crud-sub" v-if="!loading">{{ activeCount }} currenc{{ activeCount !== 1 ? 'ies' : 'y' }}</p>
      </div>
      <div class="head-actions">
        <label class="show-deleted"><input type="checkbox" v-model="showDeleted" @change="load" /> Show deleted</label>
        <button class="btn-primary" @click="openCreate">+ Add Currency</button>
      </div>
    </div>
    ...
    <div class="table-wrap">
      <table class="data-table">
        <thead>
          <tr><th>Code</th><th>Name</th><th>Order</th><th>Actions</th></tr>
        </thead>
        <tbody>
          <tr v-if="loading"><td colspan="4" class="empty-row">Loading…</td></tr>
          <tr v-else-if="items.length === 0"><td colspan="4" class="empty-row">No currencies yet.</td></tr>
          <tr v-for="c in items" :key="c.currencyId" class="data-row" :class="{ 'row-deleted': c.deletedAt }">
            <td><strong>{{ c.code }}</strong><span v-if="c.deletedAt" class="del-badge">deleted</span></td>
            <td>{{ c.name || '—' }}</td>
    ...
    <template v-if="showForm">
      ...
      <div class="drawer">
        <div class="drawer-header">
          <h2>{{ editTarget ? 'Edit' : 'Add' }} Currency</h2>
          <button class="drawer-close" @click="showForm = false">✕</button>
        </div>
        <div class="drawer-form">
          <div class="field">
            <label>Code <span class="req">*</span></label>
            <input v-model="form.code" placeholder="e.g. CHF, RM, USD" maxlength="16" />
          </div>
          <div class="field">
            <label>Name</label>
            <input v-model="form.name" placeholder="e.g. Swiss Franc" />
          </div>
          <div class="field">
    ...
    <template v-if="confirmDelete">
      ...
      <div class="confirm-modal">
        <p class="confirm-msg">Delete <strong>{{ confirmDelete.code }}</strong>? It will be soft-deleted and can be restored.</p>
...
const form = reactive({ code: '', name: '', displayOrder: 0 })
...
async function load() {
  ...
  error.value = null
  try {
    const res = await api.get(`/v1/school/currencies?includeDeleted=${showDeleted.value}`)
...
function openCreate() {
  ...
  form.code = ''; form.name = ''; form.displayOrder = 0
  formError.value = null
  showForm.value = true
}
function openEdit(c) {
  editTarget.value = c
  form.code = c.code; form.name = c.name ?? ''; form.displayOrder = c.displayOrder ?? 0
  formError.value = null
  showForm.value = true
}

async function save() {
  if (!form.code.trim()) { formError.value = 'Code is required'; return }
  saving.value = true; formError.value = null
  try {
    const body = { code: form.code.trim(), name: form.name.trim() || null, displayOrder: Number(form.displayOrder) || 0 }
    if (editTarget.value) await api.put(`/v1/school/currencies/${editTarget.value.currencyId}`, body)
    else await api.post('/v1/school/currencies', body)
...
async function doDelete(c) {
  ...
  error.value = null
  try { await api.delete(`/v1/school/currencies/${c.currencyId}`); await load() }
  catch (e) { error.value = e.response?.data?.message ?? e.message ?? 'Delete failed' }
}

async function restore(c) {
  error.value = null
  try { await api.post(`/v1/school/currencies/${c.currencyId}/restore`); await load() }
